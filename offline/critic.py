...
def expanded_gumbel_loss(diff, alpha, args=None):
    loss = 0. 
    max_log = None
    for n in range(2, args.taylor_dim+1)[::-1]:
        sign = jnp.sign(diff) if (n % 2 != 0) else jnp.ones_like(diff)
        log_term = n * jnp.log(jnp.abs(diff/alpha)+1e-8) - math.lgamma(n + 1)
        if max_log is None:
            max_log = log_term.max()
            max_log = jnp.where(max_log < 0., 0., max_log)
            max_log = jax.lax.stop_gradient(max_log)
        log_term = log_term - max_log
        current_term = sign * jnp.exp(log_term)
        loss = loss + current_term
    return loss
...
def update_v(critic: Model, value: Model, batch: Batch,
    ...
    def value_loss_fn(value_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        ...
        if vanilla:
            ...
            if args.taylor_expansion:
                value_loss = expanded_gumbel_loss(q - v, alpha=loss_temp, args=args).mean()
            if args.log_loss:
