...
def gumbel_taylor(pred, label, beta, dim):
    assert pred.shape == label.shape, "Shapes were incorrect"
    loss = 0. 
    for n in range(2, dim+1):
        loss = loss + ((label - pred)**n) / (math.factorial(n) * (beta**n))
    return loss
...
class GumbelSAC(Algorithm):
    ...
    def __init__(self, env, network_class, dataset_class, 
        ...
        self.taylor_dim = kwargs.pop('taylor_dim')
        ...
        self.beta = beta
        assert loss in {"gumbel_taylor", "gumbel", "gumbel_rescale", "mse"}
        self.loss = loss
    ...
    def _train_step(self, batch):
        ...
        if self.steps % self.value_freq == 0:
            ...
            if self.loss == "gumbel_taylor":
                value_loss_fn = partial(gumbel_taylor, beta=self.beta, dim=self.taylor_dim)
